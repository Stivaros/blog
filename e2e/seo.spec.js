...
test.describe("SEO elements", () => {
  test("homepage has correct meta tags", async ({ page }) => {
    ...
    await expect(description).toHaveAttribute(
      "content",
      "My blog, mainly about code and other nerdy stuff"
    );
    ...
    const twitterDescription = await page.locator(
  ...
  test("blog post has correct meta tags", async ({ page }) => {
    ...
    await expect(description).toHaveAttribute(
      "content",
      "The person who stumbles upon your code six months after your commit is predisposed to hate you, it's natural"
    );

    const ogTitle = await page.locator('meta[property="og:title"]');
    await expect(ogTitle).toHaveAttribute(
      "content",
      "The Developer After You, Hates You"
    );
  });
